...
class IndexController extends Controller {
	...
	*/
	...
	{
		$product = DB::table('products')->where('status',1)->where('noibat',1)->take(8)->orderBy('stt','asc')->get();
		$news = DB::table('news')->where('status',1)->where('com','dich-vu')->take(6)->orderBy('id','desc')->get();
		$news2 =DB::table('news')->where('status',1)->where('com','dao-tao')->take(3)->orderBy('id','desc')->get();
		...
		return view('templates.index_tpl', compact('com','keyword','description','title','img_share','news','about','news2'));
	}
	public function getProduct(Request $req)
	{
		$cate_pro = DB::table('product_categories')->where('status',1)->where('parent_id',0)->orderby('stt','asc')->get();
		$product = DB::table('products')->select()->where('status',1)->where('spbc',1)->orderBy('id','desc')->take(6)->get();
	...
	public function getAbout()
	{
		...
		return view('templates.about_tpl', compact('about','keyword','description','com','title'));
	}
	...
	public function getNews()
	{
		...
		$tintuc = DB::table('news')->where('status',1)->where('com', 'dich-vu')->orderBy('stt','asc')->take(9)->get();		
		$com='tin-tuc';
		// Cấu hình SEO
		$title = "Tin tức";
		$keyword = "Tin tức";
	...
	public function getListNews($id)
	{
		...
		$cateNews = DB::table('news_categories')->where('com','dich-vu')->get();
	...
	public function daoTao(){
		$daotao = DB::table('news')->where('com','dao-tao')->where('status',1)->orderBy('id','desc')->get();
		$title = 'Đào tạo';
		return view('templates.daotao', compact('daotao', 'title'));
	}
	public function daoTaoDetail($alias){
		$news_detail  = DB::table('news')->select()->where('com','dao-tao')->where('alias',$alias)->first();
		if(!empty($news_detail)){
			$com = 'dao-tao';
			...
			$description = $news_detail->description;
			$img_share = asset('upload/news/'.$news_detail->photo);
			return view('templates.daotao_detail',compact('$news_detail','$title','$newsSameCate','$keyword
				','description','img_share'));
		}else {
			return redirect()->route('getErrorNotFount');
		}
	...
	public function congNghe(){
		...
		$title = 'Chuyển giao công nghê';
		return view('templates.congnghe', compact('$data','$title'));
	}
	public function congNgheDetail($alias){
		$data = DB::table('news')->select()->where('com','chuyen-giao')->where('alias',$alias)->first();
		if(!empty($data)){
			$com = 'dao-tao';
			$newsSameCate = DB::table('news')->where('status',1)->where('com','dao-tao')->orderBy('id','desc')->get();
			if(!empty($data->title)){
			...
			return view('templates.congnghe_detail',compact('$data','$title','$newsSameCate','$keyword
				','description','img_share'));
		}else {
			return redirect()->route('getErrorNotFount');
		}
